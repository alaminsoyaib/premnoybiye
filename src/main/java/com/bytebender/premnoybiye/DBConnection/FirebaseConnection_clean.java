...
public class FirebaseConnection {
    ...
    /**
     * Upload an image to Firebase Storage (backward compatibility method)
     * This method will attempt to get user credentials from current session
     * 
     * @param imageFile The image file to upload
     * @param userId    The user ID for renaming the file
     * @return String download URL if successful, null if failed
    ...
    public String uploadImageToStorage(File imageFile, String userId) {
        // For backward compatibility, we need the user's credentials
        // Since we don't have them in this signature, we'll need to handle this differently
        System.err.println("Warning: uploadImageToStorage called without credentials.");
        System.err.println("Please use uploadImageToStorageWithToken(File, String, String) method with ID token.");
        System.err.println("This may fail due to authentication requirements.");
        
        // Try the upload without authentication (will likely fail with modern Firebase rules)
        try {
            if (imageFile == null || !imageFile.exists()) {
                System.err.println("Image file does not exist");
                return null;
            ...
            System.out.println("Uploading image to Firebase Storage (without authentication)");
            System.out.println("Original filename: " + originalName);
            System.out.println("New filename: " + newFileName);
            System.out.println("Content type: " + contentType);
    ...
    private boolean changeFirebaseAuthPassword(String idToken, String newPassword) {
        try {
            ...
            changeData.put("returnSecureToken", false);
            ...
            if (response.statusCode() == 200) {
                ...
                try {
                    ...
                    if (errorNode.has("error")) {
                        JsonNode errorDetails = errorNode.get("error");
                        String message = errorDetails.get("message").asText();
                        showErrorAlert("Password Change Failed", "Failed to change password: " + message);
                    }
                } catch (Exception e) {
                    showErrorAlert("Password Change Error", "Failed to change password. Please try again.");
                }
                System.err.println("Firebase Auth password change failed. Response: " + response.body());
                return false;
            }
    ...
    /**
     * Deletes a Firebase Authentication user account using ID token
     * 
     * @param idToken Firebase ID token for authentication
     * @return true if successful, false otherwise
     */
    private boolean deleteFirebaseAuthUserWithToken(String idToken) {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                System.err.println("Firebase Auth user deletion failed. Response: " + response.body());
                return false;
            }
    ...
    /**
     * Public method to get Firebase ID token for a user (used for authenticated operations)
     * 
     * @param email    User's email
     * @param password User's password
